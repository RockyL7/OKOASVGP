...
para_list = [-0.2, -0.1, 0.0, 1.0, 3.0, 6.0, 11.0, 16.0]

compare_list = []
touch("dataframe/test_ascg_same_seed$(seed)_density$(density).csv")
header = ["eta", "variance", "iters"]
CSV.write("dataframe/test_ascg_same_seed$(seed)_density$(density).csv", DataFrame([]); header = header)
...
for (η) in para_list
    ...
    CSV.write("dataframe/test_ascg_same_seed$(seed)_density$(density).csv", DataFrame(row,:auto); append = true)
end
