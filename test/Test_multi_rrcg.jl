...
# push!(para_list, [(0.10, 0.0, 0.0), (0.10, 10.0, 0.0), (0.10, 20.0, 0.0), (0.10, 30.0, 0.0), (0.10, 40.0, 0.0)])
# push!(para_list, [(0.05, 0.0, 0.15), (0.05, 0.0, 0.0), (0.05, 10.0, 0.0), (0.05, 20.0, 0.0), (0.05, 30.0, 0.0)])
...
compare_list = []
touch("dataframe/test_rrcg_diff_seed$(seed)_density$(density).csv")
header = ["λ", "term_min", "init_prob", "variance", "iters"]
CSV.write("dataframe/test_rrcg_diff_seed$(seed)_density$(density).csv", DataFrame([]); header = header)
...
for (para) in para_list
    ...
    for (λ, term_min, init_prob) in para
        ...
        for i = 1 : num
            temp = zeros(n)
            temp_unweighted, temp, num_iters = rrcg((x,y) -> mul!(x, A2, y), b, temp; term_min = term_min, λ = λ, init_prob = init_prob, maxIter = n)
        ...
        for i = 1 : num
            temp = zeros(n)
            temp_unweighted, temp, num_iters = rrcg((x,y) -> mul!(x, A3, y), b, temp; term_min = term_min, λ = λ, init_prob = init_prob, maxIter = n)
        ...
        CSV.write("dataframe/test_rrcg_diff_seed$(seed)_density$(density).csv", DataFrame(row,:auto); append = true)
    end
