...
function GP_ascg(K::Matrix{Float64}, train_x::Matrix{Float64}, train_y::Vector{Float64}, iter::Int64, α::Float64, γ, sche, params, dLdθ, num_sample, n, loss_list, step_list, η)
    ...
    while (i<=iter) 
        ...
        # compute K^{-1}y
        sol, weighted_sol, step = ascg((x, y)-> mul!(x, K, y), train_y, sol; η = η)
        ...
        for j = 1 : length(params) 
            ...
            for k = 1 : num_sample
                ...
                sol_2, weighted_sol_2, step = ascg((x, y)-> mul!(x, K, y), probe_vec, zeros(n); η = η)
